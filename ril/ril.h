...
typedef struct
{
    ...
    int mnc;       /* 2 or 3-digit Mobile Network Code, 0..999, INT_MAX if unknown  */
    int mnc_digit; /*2 or 3-digit*/
    int pci;       /* Physical Cell Id in range [0, 1007] */
    int tac;       /* 16-bit tracking area code, INT_MAX if unknown  */
    int nrarfcn;   /* NR Absolute Radio Frequency Channel Number, in range [0, 3279165] */
    uint64_t nci;  /* NR Cell Identity in range [0, 68719476735] (36 bits) described in 3GPP TS 38.331,
                   * LONG_MAX (2^63-1) means invalid/unreported.*/
} RIL_CellIdentityNr;
...
/* @} */
#define RIL_REQUEST_IMS_UPDATE_CLIP (RIL_IMS_REQUEST_BASE + 37)
#define RIL_REQUEST_IMS_REGADDR (RIL_IMS_REQUEST_BASE + 38)
...
#define RIL_EXT_REQUEST_SET_IMEI (RIL_EXT_REQUEST_BASE + 42)
#define RIL_EXT_REQUEST_SET_IMEISV (RIL_EXT_REQUEST_BASE + 43)
#define RIL_EXT_REQUEST_QUERY_LTE_CTCC (RIL_EXT_REQUEST_BASE + 44)
#define RIL_EXT_REQUEST_QUERY_LTE_CTCC_SINR (RIL_EXT_REQUEST_BASE + 45)
#define RIL_EXT_REQUEST_QUERY_LTE_CTCC_MMEI (RIL_EXT_REQUEST_BASE + 46)
#define RIL_EXT_REQUEST_SET_GSM_BAND (RIL_EXT_REQUEST_BASE + 47)
#define RIL_EXT_REQUEST_SET_WCDMA_BAND (RIL_EXT_REQUEST_BASE + 48)
#define RIL_EXT_REQUEST_SET_LTE_BAND (RIL_EXT_REQUEST_BASE + 49)
#ifdef ORCA_FEATURE_5G
#define RIL_EXT_REQUEST_DATA_REGISTRATION_STATE (RIL_EXT_REQUEST_BASE + 50)
#define RIL_EXT_REQUEST_EVALUATE_URSP (RIL_EXT_REQUEST_BASE + 51)
#define RIL_EXT_REQUEST_SETUP_DATA_CALL (RIL_EXT_REQUEST_BASE + 52)
#define RIL_EXT_REQUEST_GET_CELL_INFO_LIST (RIL_EXT_REQUEST_BASE + 53)
#define RIL_EXT_REQUEST_SIGNAL_STRENGTH (RIL_EXT_REQUEST_BASE + 54)
#define RIL_EXT_REQUEST_SET_NR_BAND (RIL_EXT_REQUEST_BASE + 55)
#define RIL_EXT_REQUEST_LAST RIL_EXT_REQUEST_SET_NR_BAND
#else
#define RIL_EXT_REQUEST_LAST RIL_EXT_REQUEST_SET_LTE_BAND
#endif
...
#define RIL_EXT_UNSOL_ATROUTER_RSP (RIL_EXT_UNSOL_RESPONSE_BASE + 21)
#define RIL_EXT_UNSOL_DATA_PDP_INFO (RIL_EXT_UNSOL_RESPONSE_BASE + 22)
#ifdef ORCA_FEATURE_5G
#define RIL_EXT_UNSOL_PHYSICAL_CHANNEL_CONFIG (RIL_EXT_UNSOL_RESPONSE_BASE + 23)
#define RIL_EXT_UNSOL_NETWORK_SCAN_RESULT (RIL_EXT_UNSOL_RESPONSE_BASE + 24)
#define RIL_EXT_UNSOL_SIGNAL_STRENGTH (RIL_EXT_UNSOL_RESPONSE_BASE + 25)
#define RIL_EXT_UNSOL_SIGNAL_CONN_STATUS (RIL_EXT_UNSOL_RESPONSE_BASE + 26)
#define RIL_EXT_UNSOL_PPP_RSP (RIL_EXT_UNSOL_RESPONSE_BASE + 27)
#define RIL_EXT_UNSOL_PPP_DATA_PDP_INFO (RIL_EXT_UNSOL_RESPONSE_BASE + 28)
#define RIL_EXT_UNSOL_RESPONSE_LAST RIL_EXT_UNSOL_PPP_DATA_PDP_INFO
#define RIL_EXT_UNSOL_RESPONSE_LAST RIL_EXT_UNSOL_SIGNAL_CONN_STATUS
#else
#define RIL_EXT_UNSOL_PPP_RSP (RIL_EXT_UNSOL_RESPONSE_BASE + 23)
#define RIL_EXT_UNSOL_PPP_DATA_PDP_INFO (RIL_EXT_UNSOL_RESPONSE_BASE + 24)
#define RIL_EXT_UNSOL_RESPONSE_LAST RIL_EXT_UNSOL_PPP_DATA_PDP_INFO
#endif
...
typedef struct
{
    ...
    int ipv6AddrPrefixLen; //uint8_t ipv6AddrPrefixLen;
    int tosField;          //uint8_t tosField;
    int tosMask;           //uint8_t tosMask;
    int startPort;
    int endPort;
    char *ipv4Addr;
    char *ipv4AddrMask;
...
typedef struct
{
    ...
    char *osId;
    char *appId;
    char *nonIPDescriptor;
    RIL_IPDescriptorInfo ipDescriptorInfo;
    char *domainDescriptor;
    int dnnNumber;
    char **dnn;
} RIL_TrafficDescriptorInfo;
...
typedef struct
{
    ...
    int id;
    int sscMode;
    char *dnn;    // char dnn[100];
    char *snssai; //char snssai[32];
} RIL_RouteSelectionDescriptorInfo;
...
typedef struct
{
    ...
    RIL_DataProfileInfo_v15 dataProfileInfo;
    RIL_TrafficDescriptorInfo td;
    RIL_RouteSelectionDescriptorInfo rsd;
} RIL_RequestSetupDataCall;
